...
object Desktop {
    ...
    private fun browseDesktop(uri: URI): Boolean {
        return try {
            if (!Desktop.isDesktopSupported()) {
                Installer.logger.debug("Desktop is not supported. OS: " + System.getProperty("os.name"))
                return false
            }
            ...
            Installer.logger.debug("Attempting to open $uri with Desktop#browse()")
    ...
    private fun openDesktop(file: File): Boolean {
        return try {
            ...
            if (!Desktop.getDesktop().isSupported(Desktop.Action.OPEN)) {
                Installer.logger.warn("OPEN is not supported.")
                return false
            }

            Installer.logger.debug("Attempting to open $file with Desktop#open()")
    ...
    private fun editDesktop(file: File): Boolean {
        return try {
            ...
            Installer.logger.debug("Attempting to edit $file with Desktop#edit()")
    ...
    private fun runCommand(command: String, args: String, file: String): Boolean {
        Installer.logger.debug("Trying to exec:\n   cmd = $command\n   args = $args\n   %s = $file")
